...
This is news at your fingertips. It's simply an RSS news aggregator across DS blogs around the web. It's not ranked or personalized, just sorted by publication date. Each table can be filtered by author (using `Feed` search), searched using regular expresssions (table search), or sorted by date or post. 
...
process_feed <- function(df){
  df %>% 
    ...
    mutate(rand=runif(n())) %>%
    select(blog, post, blog_title, feed_title, rand) 
}
...
df2 <- df1 %>% rbind(datatau)
df3 <- process_feed(df2)

# produce table
sd <- SharedData$new(df3)
my_bscols(sd)
make_dt(sd)
